...
export default class QQShare extends Component {
      ...
      <View style={styles.container}>
        ...
        >
          ...
          {Platform.OS === 'ios' ? null :
            (<Button
              onPress={this.shareImageFromLocal.bind(this)}
              title="分享相册照片"
              color="#841584"
            />)
          }
  ...
  shareImageFromLocal() {
    CameraRoll.getPhotos({first: 1, assetType: 'Photos'})
      .then((asset) => {
        console.log(asset.edges[0].node.image.uri);
        QQ.shareImage(asset.edges[0].node.image.uri, '分享图片的标题', '分享图片的描述', QQ.shareScene.QQ)
          .then((result) => {
            Alert.alert('QQ分享结果', ' ' + result);
          })
          .catch((error) => {
  ...
  shareNews() {
    ...
    QQ.shareNews(newsUrl, resolveAssetSource(require('./news.jpg')).uri, '分享新闻的标题', '分享新闻的描述', QQ.shareScene.QQ)
      .then((result) => {
        Alert.alert('QQ分享结果', ' ' + result);
      })
      .catch((error) => {
